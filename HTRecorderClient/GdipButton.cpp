...
BOOL CGdipButton::LoadStdImage(WCHAR * file)
{
	m_pStdImage = new CGdiPlusBitmapResource;
	//m_pStdImage->Load()
	return m_pStdImage->Load(file);
}

BOOL CGdipButton::LoadAltImage(WCHAR * file)
{
	m_bHaveAltImage = TRUE;
	m_pAltImage = new CGdiPlusBitmapResource;
	return (m_pAltImage->Load(file));
}
